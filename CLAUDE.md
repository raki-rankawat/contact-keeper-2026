...
There is no test runner, linter, or formatter configured. `concurrently` sits in devDependencies for a client app that does not exist yet. All three are specced in [features/04-tooling.md](features/04-tooling.md) — note that adding a formatter contradicts a convention stated below and needs the user's decision first.
...
[features/](features/) holds written specs for planned changes — problem, fix, and acceptance criteria per item. **None of it is implemented.** The Architecture and Gotchas sections here describe the code as it actually is.

[features/00-implementation-plan.md](features/00-implementation-plan.md) sequences everything by dependency and marks where to stop. Check it before starting non-trivial work: several items are ordered deliberately (breaking changes batched before a client exists, the app/server split shared between the test harness and graceful shutdown), and doing them out of order means doing them twice.
...
- No semicolons, single quotes, 2-space indent, trailing commas, arrow functions. No Prettier config — match surrounding files. (Adding one is proposed in [T5](features/04-tooling.md) but not agreed; until it is, this line is the rule.)
